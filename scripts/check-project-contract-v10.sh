...
work=$(mktemp -d "${TMPDIR:-/tmp}/seme-project-v10.XXXXXX")
trap 'rm -rf "$work"' EXIT HUP INT TERM
(cd "$repo/reference/go" && GOCACHE="$work/cache" go test -count=1 ./projectmodule ./contractcatalog && GOCACHE="$work/cache" go run ./cmd/project-module -version 10 > "$work/module.g1")
cmp "$repo/modules/project/v10/module.g1" "$work/module.g1"

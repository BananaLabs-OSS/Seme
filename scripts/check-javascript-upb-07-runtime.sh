...
vectors="$fixture/durable-vectors.json"
pulp_repo=${PULP_REPO:-"$repo/../Pulp"}; pulp_commit=acc66ca61fe69c5f2c4093bc55e13aeac6dcc001
work=$(mktemp -d "${TMPDIR:-/tmp}/seme-javascript-upb07-runtime.XXXXXX")
trap 'rm -rf "$work"' EXIT HUP INT TERM
GOCACHE="$work/go-cache"; XDG_CACHE_HOME="$work/cache"; export GOCACHE XDG_CACHE_HOME

(cd "$fixture" && node "$repo/reference/js/javascript-package-provider-cli.mjs" --files application.js,configuration.js,policy.js,state.js \
...
fs.writeFileSync(process.argv[3], v.valid.map(x=>JSON.stringify({arguments:x.arguments})).join("\n")+"\n");
...
cmp "$work/expected.jsonl" "$work/wasm.jsonl"
...
cp "$repo/targets/wasm/pulp-canonical-vm-v1/pulp.cell.toml" "$work/pulp/pulp.cell.toml"; cp "$work/vm.wasm" "$work/pulp/canonical-vm.wasm"
...
cmp "$work/expected.jsonl" "$work/pulp.jsonl"

(cd "$repo/reference/go" && go test -race -count=1 ./durableruntime && go test -count=10 ./durableruntime)
if rg -n '(^|[^[:alnum:]_])(os|time|rand)\.|database/sql|sqlite|go[[:space:]]+func' "$repo/reference/go/durableruntime" --glob '*.go' | rg -v '_test\.go:'; then
  echo 'durable runtime boundary gained ambient/provider-specific behavior' >&2; exit 1
fi
echo 'JavaScript UPB-07 runtime: native JS, canonical Seme, Wasm, and pinned Pulp migration agree; neutral DurablePort host boundary passes'

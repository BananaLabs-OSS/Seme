...
work=$(mktemp -d "${TMPDIR:-/tmp}/seme-execution-v71.XXXXXX")
...
(cd "$repo/reference/go" && go run -buildvcs=false ./cmd/execution-module-v71) > "$work/module.g1"
cmp "$repo/modules/execution/v71/module.g1" "$work/module.g1"
(cd "$repo/modules/execution/v71" && sha256sum -c module.g1.sha256 && sha256sum -c module.seme.sha256)
"$repo/bootstrap/seme-k0-linux-amd64" "$repo/compiler/g1-compiler.k0" "$work/module.g1" "$work/module.seme"
cmp "$repo/modules/execution/v71/module.seme" "$work/module.seme"
version=2
...
echo 'Core Execution v71: typed native slicing is reproducible'

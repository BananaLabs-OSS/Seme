...
work=$(mktemp -d "${TMPDIR:-/tmp}/seme-execution-v8.XXXXXX")
...
execution="$repo/modules/execution/v8"

(
    cd "$repo/reference/go"
    go test ./executionmodule ./goprovider
    go run ./cmd/execution-module-v8 > "$work/module.g1"
    go build -o "$work/go-provider" ./cmd/go-provider
    go build -o "$work/go-lift" ./cmd/go-execution-lift
    go build -o "$work/structured-check" ./cmd/structured-v8-check
)
...
cp -R "$repo/fixtures/go-execution-v8" "$work/project"
...
"$work/go-lift" \
    ...
    --module "$execution/module.g1" --function Combine \
...
# Frozen predecessor artifacts remain reproducible from the same emitter.
version=2
while [ "$version" -le 7 ]; do
...
echo "Core Execution v8: generic Block/Return composition passed"

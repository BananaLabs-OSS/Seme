#!/bin/sh
# Real cumulative UPB09 native, canonical, standalone Wasm, and pinned Pulp
# parity over one reproducible 4,096-observation controlled-effects corpus.
set -eu
repo=$(CDPATH= cd -- "$(dirname -- "$0")/.." && pwd)
work=$(mktemp -d "${TMPDIR:-/tmp}/seme-go-upb09-runtime.XXXXXX")
...
"$repo/scripts/materialize-go-upb09-fixture.sh" "$work/project"
for corpus in corpus-a corpus-b; do
  (cd "$work/project" && GOTOOLCHAIN=local GOPROXY="file://$proxy" GOSUMDB=off \
    go test -count=1 -buildvcs=false ./streamservice \
      -run '^TestNativeControlledEffectsRuntimeCorpus$' \
      -args -native-effects-corpus-dir "$work/$corpus")
done
...
(cd "$repo/reference/go" &&
  go build -buildvcs=false -o "$work/lift" ./cmd/go-session-proof &&
...
"$work/lift" -module "$repo/modules/execution/v36/module.g1" -project "$work/project" \
  -package example.test/go-uab-11/streamservice -entry DispatchControlled -out "$work/program.g1"
...
node "$repo/reference/js/javascript-uab-11-jsonl-equal.mjs" \
  "$work/corpus-a/expected.jsonl" "$work/canonical.jsonl"

"$work/codec" encode "$work/program.seme" < "$work/corpus-a/requests.jsonl" > "$work/requests.hex"
node "$repo/reference/js/canonical-wasm-cell-runner.mjs" \
  "$work/vm.wasm" "$work/program.seme" "$work/requests.hex" > "$work/wasm.hex"
"$work/codec" observe "$work/program.seme" < "$work/wasm.hex" > "$work/wasm.jsonl"
node "$repo/reference/js/javascript-uab-11-jsonl-equal.mjs" \
  "$work/corpus-a/expected.jsonl" "$work/wasm.jsonl"
...
cp "$repo/targets/wasm/pulp-canonical-vm-v1/runner.go" \
  "$work/pinned/cmd/pulp-seme-canonical-vm-proof/main.go"
...
node "$repo/reference/js/javascript-uab-11-jsonl-equal.mjs" \
  "$work/corpus-a/expected.jsonl" "$work/pulp.jsonl"

printf 'Go UPB-09 native, canonical, standalone Wasm, and pinned Pulp parity passes over 4,096 controlled-effects observations\n'

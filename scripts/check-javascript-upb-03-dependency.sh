...
resolve() { source=$1; output=$2; node "$repo/reference/js/javascript-npm-resolver-cli.mjs" --manifest "$source/package.json" --lock "$source/package-lock.json" \
  --local-root "$source" --local-files math/sum.js --external-root "$external" --external-files index.js,package.json \
...
"$work/project-roundtrip-publish" -root "$fixture" -snapshot "$work/snapshot.json" -dest "$work/projected-native" \
  -projected "application.js=$work/generated-modules/application.js" -projected "math/sum.js=$work/generated-modules/math/sum.js" \
...
node --check "$work/projected-native/application.js"; node --check "$work/projected-native/math/sum.js"

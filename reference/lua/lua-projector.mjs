...
export function projectLua(canonicalG1) {
  ...
  const resolveType = (id, visiting = new Set()) => {
    ...
    if(item.schema===schema.nativeType)name=`seme.native<${text(field(item,0xa0710))},${text(field(item,0xa0711))}>`;
  ...
  const context = { graph, names, types, interfaces, methods, witnesses };
	const scopedRecordIDs = new Set([...graph.values()].filter((item) => item.schema === schema.scopedTypeDeclaration).map((item) => reference(field(item, 0xa0810))));
  const records = [...graph.values()].filter((item) => item.schema === schema.recordType && !scopedRecordIDs.has(item.id)).map((record) => {
...
function projectFunction(id, exported, context) {
  ...
  const local = { ...context, parameters: new Map(parameters.map((item) => [item.id, item])), places: new Map(), locals:new Map() };
  const identity=`---@seme-id ${id}`;
...
function projectControlBlock(id,context,depth){const indent="  ".repeat(depth),lines=[],statementIDs=references(field(required(context.graph,id,schema.block),0x9800)),skip=new Set(),stateful=new Map();for(const statementID of statementIDs){const s=required(context.graph,statementID);if(s.schema!==schema.bindLocal)continue;const localID=reference(field(s,0x9d10)),local=required(context.graph,localID,schema.localBinding),initializer=required(context.graph,reference(field(local,0x9d02)));if(initializer.schema===schema.statefulIndirectCall)stateful.set(localID,{statementID,local,initializer,result:null,assignment:null});}for(const statementID of statementIDs){const s=required(context.graph,statementID);if(s.schema===schema.bindLocal){const resultLocalID=reference(field(s,0x9d10)),local=required(context.graph,resultLocalID,schema.localBinding),initializer=required(context.graph,reference(field(local,0x9d02)));if(initializer.schema!==schema.transitionResult)continue;const read=required(context.graph,reference(field(initializer,0xa0070)),schema.localRead),pending=stateful.get(reference(field(read,0x9d20)));if(pending){pending.result={statementID,id:resultLocalID,name:text(field(local,0x9d00))};skip.add(statementID);}}else if(s.schema===schema.assignPlace){const value=required(context.graph,reference(field(s,0x9e31)));if(value.schema!==schema.transitionResult)continue;const read=required(context.graph,reference(field(value,0xa0070)),schema.localRead),pending=stateful.get(reference(field(read,0x9d20))),placeEntity=required(context.graph,reference(field(s,0x9e30)),schema.mutablePlace),place=text(field(placeEntity,0x9e00));if(pending&&place){pending.assignment=place;skip.add(statementID);}}}for(const statementID of statementIDs){if(skip.has(statementID))continue;const s=required(context.graph,statementID);
  if(s.schema===schema.scopedTypeDeclaration){const typeID=reference(field(s,0xa0810)),record=required(context.graph,typeID,schema.recordType),name=text(field(record,0x9300)),members=references(field(record,0x9301)).map(id=>text(field(required(context.graph,id,schema.recordField),0x9310)));lines.push(`${indent}Seme.scoped_type(${JSON.stringify(typeID)}, ${JSON.stringify(name)}, { ${members.map(JSON.stringify).join(", ")} })`);continue;}
  ...
  if(s.schema===schema.nativeRange){const language=text(field(s,0xa07c0)),collection=projectExpression(reference(field(s,0xa07c1)),context),child={...context,locals:new Map(context.locals)},bind=(fieldID,fallback)=>{const ids=references(field(s,fieldID));if(ids.length>1)fail("lua_projection.native_range_binding");if(!ids.length)return fallback;const item=required(context.graph,ids[0],schema.nativeRangeBinding),name=text(field(item,0xa07b0));child.locals.set(ids[0],name);return name;},key=bind(0xa07c2,"_key"),value=bind(0xa07c3,"_value");lines.push(`${indent}Seme.native_range(${JSON.stringify(language)}, ${collection}, function(${key}, ${value})`);lines.push(projectControlBlock(reference(field(s,0xa07c4)),child,depth+1));lines.push(`${indent}end)`);continue;}
  if(s.schema===schema.nativeDefer){const language=text(field(s,0xa0750)),invocation=projectExpression(reference(field(s,0xa0751)),context);lines.push(`${indent}Seme.defer_native(${JSON.stringify(language)}, function() return ${invocation} end)`);continue;}
  if(s.schema===schema.nativeFieldAssignment){const language=text(field(s,0xa0760)),member=text(field(s,0xa0761)),receiver=projectExpression(reference(field(s,0xa0762)),context),value=projectExpression(reference(field(s,0xa0763)),context);lines.push(`${indent}Seme.assign_native_field(${JSON.stringify(language)}, ${receiver}, ${JSON.stringify(member)}, ${value})`);continue;}
  if(s.schema===schema.nativeIndexAssignment){const language=text(field(s,0xa0800)),collection=projectExpression(reference(field(s,0xa0801)),context),index=projectExpression(reference(field(s,0xa0802)),context),value=projectExpression(reference(field(s,0xa0803)),context);lines.push(`${indent}Seme.assign_native_index(${JSON.stringify(language)}, ${collection}, ${index}, ${value})`);continue;}
  if(s.schema===schema.nativeSwitch){const language=text(field(s,0xa0780)),subjectID=reference(field(s,0xa0781)),subject=required(context.graph,subjectID).schema===schema.unitValue?"nil":projectExpression(subjectID,context),caseIDs=references(field(s,0xa0782)),defaults=references(field(s,0xa0783));if(defaults.length>1)fail("lua_projection.native_switch_default");lines.push(`${indent}Seme.native_switch(${JSON.stringify(language)}, ${subject}, {`);for(const caseID of caseIDs){const branch=required(context.graph,caseID,schema.nativeSwitchCase),values=references(field(branch,0xa0770)).map(id=>projectExpression(id,context)).join(", ");lines.push(`${indent}  { values = { ${values} }, run = function()`);lines.push(projectControlBlock(reference(field(branch,0xa0771)),{...context,places:new Map(context.places),locals:new Map(context.locals)},depth+2));lines.push(`${indent}  end },`);}lines.push(`${indent}}, function()`);if(defaults.length===1)lines.push(projectControlBlock(defaults[0],{...context,places:new Map(context.places),locals:new Map(context.locals)},depth+1));lines.push(`${indent}end)`);continue;}
  if(s.schema===schema.nativeBranch){lines.push(`${indent}Seme.native_branch(${JSON.stringify(text(field(s,0xa0790)))}, ${JSON.stringify(text(field(s,0xa0791)))}, ${JSON.stringify(text(field(s,0xa0792)))})`);continue;}
...
function projectProtocolDispatch(branch, context) {
  const unpack = (blockID) => { const statements = references(field(required(context.graph, blockID, schema.block), 0x9800)); if (statements.length !== 1) fail("lua_projection.protocol_branch"); const values = references(field(required(context.graph, statements[0], schema.returned), 0x9810)); const call = required(context.graph, values[0], schema.dynamicMethodCall), boxed = required(context.graph, reference(field(call, 0xa0140)), schema.interfaceValue), valueID=reference(field(boxed,0xa0131)), value=required(context.graph,valueID), construct=value.schema===schema.recordConstruct?value:null, witness = context.witnesses.get(reference(field(boxed, 0xa0132))); if (!witness) fail("lua_projection.protocol_witness"); return { call, boxed, valueID, construct, witness }; };
  ...
  const args = references(field(yes.call, 0xa0142)), noArgs = references(field(no.call, 0xa0142));
  ...
  if (!yes.construct || !no.construct) {
    if (yes.construct || no.construct || args.length !== 1 || noArgs.length !== 1) fail("lua_projection.protocol_scalar");
    return `Seme.protocol_dispatch_value(${projectExpression(reference(field(branch,0x9c00)),context)}, ${binding(no.witness)}, ${binding(yes.witness)}, ${JSON.stringify(requirement.name)}, ${projectExpression(yes.valueID,context)}, ${projectExpression(args[0],context)})`;
  }
  ...
  const values = references(field(yes.construct, 0x9331)), noValues = references(field(no.construct, 0x9331));
...
function projectExpression(id, context) {
  ...
  if(expression.schema===schema.nativeBinary)return `Seme.native_binary(${JSON.stringify(text(field(expression,0xa07f0)))}, ${JSON.stringify(text(field(expression,0xa07f1)))}, ${projectExpression(reference(field(expression,0xa07f2)),context)}, ${projectExpression(reference(field(expression,0xa07f3)),context)}, ${JSON.stringify(reference(field(expression,0xa07f4)))})`;
  ...
  if(expression.schema===schema.localRead){const name=context.locals?.get(reference(field(expression,0x9d20)));if(!name)fail("lua_projection.local_read_scope");return name;}
  ...
  if(expression.schema===schema.recordConstruct){const record=required(context.graph,reference(field(expression,0x9330)),schema.recordType),name=text(field(record,0x9300)),members=references(field(record,0x9301)).map(id=>required(context.graph,id,schema.recordField)),values=references(field(expression,0x9331));if(values.length!==members.length)fail("lua_projection.record_construct");const names=members.map(item=>text(field(item,0x9310)));return `Seme.record(${JSON.stringify(name)}, { ${names.map(JSON.stringify).join(", ")} }, { ${names.map((item,index)=>`${item} = ${projectExpression(values[index],context)}`).join(", ")} })`;}
  if(expression.schema===schema.stateTransition)return `Seme.transition(${projectExpression(reference(field(expression,0xa0051)),context)}, ${projectExpression(reference(field(expression,0xa0052)),context)})`;
  ...
  if(expression.schema===schema.mapLookupOption)return `Seme.map_lookup(${projectExpression(reference(field(expression,0xa0440)),context)}, ${projectExpression(reference(field(expression,0xa0441)),context)})`;
  ...
  if (expression.schema === schema.optionMatch) {
    ...
    return `Seme.match_option(${projectExpression(reference(field(expression, 0xa0630)), context)}, ${none}, function(${bindingName(some, context)}) return ${projectBlockExpression(reference(field(expression, 0xa0633)), context)} end)`;
  }
...
function signedI64(value){const word=BigInt(unsigned(value));if(word>(1n<<64n)-1n)fail("lua_projection.i64_literal");return BigInt.asIntN(64,word).toString();}

...
function controlExpressionType(expression, context) {
  ...
  if(expression.kind==="text_literal")return "text";
...
function emitControlExpression(expression, expected, context, path) {
  ...
  if(expression.kind==="text_literal"){const id=stableID("execution",context.description.id,path,"text-literal");context.additions.push(graphEntity(id,entity(id,schema.stringLiteral,[[0x9500,bytes(expression.value)]])));return id;}

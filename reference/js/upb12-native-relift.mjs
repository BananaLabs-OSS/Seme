...
export function reliftUPB12Native({authorityRoot,graphPath,projectRoot,language}){
  ...
  const expected=language==="javascript"?projectJavaScriptModules(canonical.toString("utf8"),graph):projectLuaModules(canonical.toString("utf8"),graph),extension=language==="javascript"?".js":".lua",repo=path.resolve(path.dirname(fileURLToPath(import.meta.url)),"../.."),detached=language==="javascript"?{"native-test.mjs":path.join(repo,"reference/upb12/native/javascript-native-test.mjs")}:{"native-test.lua":path.join(repo,"reference/upb12/native/lua-native-test.lua"),"seme-values.lua":path.join(repo,"reference/lua/seme-values.lua")};
  const found=[];walk(projectRoot,"",found);for(const name of found){if(name.endsWith(extension)&&!Object.hasOwn(detached,name)&&!Object.hasOwn(expected,name))fail(`unexpected_source:${name}`)}
  ...
  for(const[name,source]of Object.entries(detached))if(!read(path.join(projectRoot,name)).equals(read(source)))fail(`native_test_mismatch:${name}`);

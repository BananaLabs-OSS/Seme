...
if (!wasmPath || !vectorsPath) throw new Error("usage: go-uab-04-wasm-runner WASM VECTORS [--tsv]");
...
const rows = [], valid = {}, rejected = {};
for (const item of vectors.valid) {
  const request = encode(item), result = await invoke(request);
  if (result.status || result.value !== item.result) throw new Error(`go.uab04.wasm:${item.name}`);
...
for (const item of vectors.malformed) {
  ...
  if ((await invoke(request)).status === 0) throw new Error(`go.uab04.malformed:${item.name}`);
...
else console.log(JSON.stringify({valid, malformed:vectors.malformed.length, rejected}));

...
const observations = {
  none: (await invoke(none)).value === 1,
  ok: (await invoke(some(0, Buffer.from("ok")))).value === 1,
  wrong_bytes: (await invoke(some(0, Buffer.from("no")))).value === 1,
  error: (await invoke(some(1, Buffer.from("bad")))).value === 1,
  wrong_error: (await invoke(some(1, Buffer.from("no")))).value === 1,
};
assert.deepEqual(observations, { none: false, ok: true, wrong_bytes: false, error: true, wrong_error: false });
...
if (process.argv[3] === "--json") console.log(JSON.stringify(observations));
else console.log("composite Wasm: valid variants executed and malformed encodings rejected");

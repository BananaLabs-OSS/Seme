...
export function projectJavaScriptModules(canonicalG1, graph) {
  ...
  for(const item of ast.body){const declaration=item.type==="ExportNamedDeclaration"?item.declaration:item;if(declaration?.type!=="ClassDeclaration")continue;const prior=[...comments].reverse().find((comment)=>comment.type==="Block"&&comment.end<=item.start&&/^\s*$/.test(whole.slice(comment.end,item.start))&&/@typedef\s+\{Object\}/.test(comment.value));if(!prior)fail(`record_comment:${declaration.id.name}`);let source=whole.slice(prior.start,item.end);const offset=item.start-prior.start;if(item.type==="ExportNamedDeclaration")source=`${source.slice(0,offset)}${source.slice(offset).replace(/^export\s+/,"")}`;records.set(declaration.id.name,source);}

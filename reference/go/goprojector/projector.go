...
func projectBlock(id string, c context) (string, error) {
	...
	for index, statementID := range statements {
		...
		case sNativeBindingAssign:
			if language, err := text(statement, "000000000000000000000000000a0820"); err != nil || language != "go" {
				return "", fmt.Errorf("go_projection.native_binding_assignment_language")
			}
			name, err := text(statement, "000000000000000000000000000a0821")
			if err != nil || !identifier(name) {
				return "", fmt.Errorf("go_projection.native_binding_assignment_binding")
			}
			valueID, err := ref(statement, "000000000000000000000000000a0822")
			if err != nil {
				return "", err
			}
			value, err := expr(valueID, c)
			...
			lines = append(lines, "\t"+name+" = "+value)
		case sNativeDerefAssign:
			if language, err := text(statement, "000000000000000000000000000a0830"); err != nil || language != "go" {
				return "", fmt.Errorf("go_projection.native_dereference_assignment_language")
			}
			pointerID, err := ref(statement, "000000000000000000000000000a0831")
			if err != nil {
				return "", err
			}
			valueID, err := ref(statement, "000000000000000000000000000a0832")
			if err != nil {
				return "", err
			}
			pointer, err := expr(pointerID, c)
			if err != nil {
				return "", err
			}
			value, err := expr(valueID, c)
		...
		case sNativeChannelReceive:
			if language, err := text(statement, "000000000000000000000000000a0880"); err != nil || language != "go" {
				return "", fmt.Errorf("go_projection.native_channel_receive_language")
			}
			channelID, err := ref(statement, "000000000000000000000000000a0881")
			if err != nil {
				return "", err
			}
			channel, err := expr(channelID, c)

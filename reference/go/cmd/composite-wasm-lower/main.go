...
func main() {
	if len(os.Args) != 4 {
		fatal(fmt.Errorf("usage: composite-wasm-lower PROGRAM.seme OUTPUT.wasm ABI.json"))
	}
	program, err := os.ReadFile(os.Args[1])
	fatal(err)
	graph, err := wire.Decode(program)
	fatal(err)
	certificate, err := wasmtarget.CertifyCompositeFunction(graph)
	fatal(err)
	wasm, abi, err := wasmtarget.LowerCertifiedCompositeFunction(certificate)
	fatal(err)
...
func fatal(err error) {
	if err != nil {
		fmt.Fprintln(os.Stderr, "composite-wasm-lower:", err)
		os.Exit(65)
	}
}

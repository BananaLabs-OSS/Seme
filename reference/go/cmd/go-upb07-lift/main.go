// Command go-upb07-lift is a fixture-only evidence adapter for the cumulative
// durable planner. It is not a general filesystem project loader.
package main
...
func main() {
	project := flag.String("project", "", "materialized UPB-07 project")
	...
	result := session.Apply(goprovider.DocumentSnapshot{Revision: 1, ModulePath: "example.test/go-uab-11", PackagePath: "example.test/go-uab-11/persistence", Entry: "BuildPlan", Files: files})
	if !result.Valid {
...
func fatal(message string) {
	fmt.Fprintln(os.Stderr, "go-upb07-lift:", message)
	os.Exit(1)
}

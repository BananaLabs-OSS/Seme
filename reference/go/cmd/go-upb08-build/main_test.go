...
func TestUPB08InputsIncludeTransportSelectionAndV11Contracts(t *testing.T) {
	o := options{transportSelection: "/selection", orderedTransportV1: "/transport", projectV11: "/v11"}
	got := o.inputs()
	for _, want := range []string{o.transportSelection, o.orderedTransportV1, o.projectV11} {
		found := false
		for _, value := range got {
			found = found || value == want
		}
		if !found {
			t.Fatalf("missing UPB08 input %q", want)
		}
	}
}

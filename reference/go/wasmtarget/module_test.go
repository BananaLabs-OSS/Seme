...
func TestModuleIsDeterministicAndLayoutDerived(t *testing.T) {
	canonical := applicationLayout{requestOffsets: [3]uint64{0, 8, 16}, requestHeaderSize: 32, responseBoolOffset: 1, responseHeaderSize: 10}
	...
	reordered := applicationLayout{requestOffsets: [3]uint64{0, 16, 8}, requestHeaderSize: 32, responseBoolOffset: 1, responseHeaderSize: 10}
...
func TestModuleRejectsUnboundedLayout(t *testing.T) {
	_, err := module(applicationLayout{requestHeaderSize: 65536, responseHeaderSize: 10})
	if err == nil {

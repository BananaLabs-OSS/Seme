...
func module(layout applicationLayout) ([]byte, error) {
	if layout.requestSize > 65535 || layout.responseSize > 65535 {
...
func providerBody(layout applicationLayout) []byte {
	...
	body.Write([]byte{1, 1, 0x7f}) // one i32 local: accepted
	body.Write([]byte{0x20, 3, 0x41})
	sleb(&body, int64(layout.requestSize))

...
func Declarations(version int) ([]Schema, error) {
	...
	if version >= 61 {
		schemas = append(schemas, Schema{0xa075, "NativeDefer", []Field{
			field(0xa0750, "native_defer.language", 4, 0, 0),
			field(0xa0751, "native_defer.invocation", 5, 0, 0),
		}})
	}
	if version >= 63 {
		schemas = append(schemas, Schema{0xa076, "NativeFieldAssignment", []Field{
			field(0xa0760, "native_field_assignment.language", 4, 0, 0),
			field(0xa0761, "native_field_assignment.field", 4, 0, 0),
	...
	if version >= 66 {
		schemas = append(schemas, Schema{0xa079, "NativeBranch", []Field{
			field(0xa0790, "native_branch.language", 4, 0, 0),
			field(0xa0791, "native_branch.operation", 4, 0, 0),
			field(0xa0792, "native_branch.target", 4, 0, 0),
		}})
	}
	if version >= 67 {
		schemas = append(schemas, Schema{0xa07a, "NativeAddress", []Field{
			field(0xa07a0, "native_address.language", 4, 0, 0),
			field(0xa07a1, "native_address.operand", 5, 0, 0),
			field(0xa07a2, "native_address.result_type", 5, 0xa071, 0),
		}})
	}
	if version >= 70 {
		schemas = append(schemas,
			Schema{0xa07b, "NativeRangeBinding", []Field{
				field(0xa07b0, "native_range_binding.name", 4, 0, 0),
				field(0xa07b1, "native_range_binding.type", 5, 0, 0),
			}},
			Schema{0xa07c, "NativeRange", []Field{
				field(0xa07c0, "native_range.language", 4, 0, 0),
				field(0xa07c1, "native_range.collection", 5, 0, 0),
				field(0xa07c2, "native_range.key_binding", 5, 0xa07b, 2),
				field(0xa07c3, "native_range.value_binding", 5, 0xa07b, 2),
				field(0xa07c4, "native_range.body", 5, 0x9080, 0),
			}},
		)
	}
	if version >= 71 {
		schemas = append(schemas, Schema{0xa07d, "NativeSlice", []Field{
			field(0xa07d0, "native_slice.language", 4, 0, 0), field(0xa07d1, "native_slice.collection", 5, 0, 0),
			field(0xa07d2, "native_slice.low", 5, 0, 2), field(0xa07d3, "native_slice.high", 5, 0, 2), field(0xa07d4, "native_slice.max", 5, 0, 2),
			field(0xa07d5, "native_slice.result_type", 5, 0, 0),
		}})
	}
	if version >= 72 {
		schemas = append(schemas, Schema{0xa07e, "NativeDereference", []Field{
	...
	if version >= 74 {
		schemas = append(schemas, Schema{0xa080, "NativeIndexAssignment", []Field{
			field(0xa0800, "native_index_assignment.language", 4, 0, 0),
			field(0xa0801, "native_index_assignment.collection", 5, 0, 0),
			field(0xa0802, "native_index_assignment.index", 5, 0, 0),
			field(0xa0803, "native_index_assignment.value", 5, 0, 0),
		}})
	}
	if version >= 81 {
		schemas = append(schemas, Schema{0xa081, "ScopedTypeDeclaration", []Field{
			field(0xa0810, "scoped_type_declaration.type", 5, 0, 0),
		}})
	}
	if version >= 94 {
		schemas = append(schemas, Schema{0xa082, "NativeBindingAssignment", []Field{
			field(0xa0820, "native_binding_assignment.language", 4, 0, 0),
			field(0xa0821, "native_binding_assignment.binding", 4, 0, 0),
			field(0xa0822, "native_binding_assignment.value", 5, 0, 0),
		}})
	}
	if version >= 95 {
		schemas = append(schemas, Schema{0xa083, "NativeDereferenceAssignment", []Field{
			field(0xa0830, "native_dereference_assignment.language", 4, 0, 0),
			field(0xa0831, "native_dereference_assignment.pointer", 5, 0, 0),
			field(0xa0832, "native_dereference_assignment.value", 5, 0, 0),
		}})
	}
	if version >= 99 {
		schemas = append(schemas, Schema{0xa084, "NativeConcurrentStart", []Field{
			field(0xa0840, "native_concurrent_start.language", 4, 0, 0),
			field(0xa0841, "native_concurrent_start.invocation", 5, 0, 0),
		}})
	}
	if version >= 100 {
		schemas = append(schemas, Schema{0xa085, "NativeChannelSend", []Field{
			field(0xa0850, "native_channel_send.language", 4, 0, 0),
			field(0xa0851, "native_channel_send.channel", 5, 0, 0),
			field(0xa0852, "native_channel_send.value", 5, 0, 0),
		}})
	}
	if version >= 101 {
	...
	if version >= 105 {
		schemas = append(schemas, Schema{0xa089, "NativeMethodValue", []Field{
			field(0xa0890, "native_method_value.language", 4, 0, 0),
			field(0xa0891, "native_method_value.method", 4, 0, 0),
			field(0xa0892, "native_method_value.signature", 4, 0, 0),
			field(0xa0893, "native_method_value.receiver", 5, 0, 0),
			field(0xa0894, "native_method_value.result_type", 5, 0xa071, 0),
		}})
	}
	if version >= 107 {
		schemas = append(schemas, Schema{0xa08a, "NativeIndirectInvocation", []Field{
			field(0xa08a0, "native_indirect_invocation.language", 4, 0, 0),
			field(0xa08a1, "native_indirect_invocation.signature", 4, 0, 0),
	...
	if version >= 114 {
		schemas = append(schemas, Schema{0xa08c, "NativeMethodExpression", []Field{
			field(0xa08c0, "native_method_expression.language", 4, 0, 0),
			field(0xa08c1, "native_method_expression.method", 4, 0, 0),
			field(0xa08c2, "native_method_expression.receiver_type", 5, 0xa071, 0),
			field(0xa08c3, "native_method_expression.result_type", 5, 0xa071, 0),
		}})
	}
	if version >= 115 {
		schemas = append(schemas, Schema{0xa08d, "NativeLexicalClosure", []Field{
			field(0xa08d0, "native_lexical_closure.language", 4, 0, 0),
			field(0xa08d1, "native_lexical_closure.signature", 4, 0, 0),
			field(0xa08d2, "native_lexical_closure.source", 4, 0, 0),
			field(0xa08d3, "native_lexical_closure.result_type", 5, 0, 0),
		}})
	}
	return schemas, nil
}

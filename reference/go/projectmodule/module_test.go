...
func TestEmitV2ExtendsV1WithExactAncestryAndNeutralSources(t *testing.T) {
	...
	if err := EmitVersion(&bytes.Buffer{}, 15); err == nil {
		t.Fatal("accepted unknown version")
	}
}

func TestEmitV14BindsExactV13PatchAndLanguageService(t *testing.T) {
	var first, second bytes.Buffer
	if err := EmitVersion(&first, 14); err != nil {
		t.Fatal(err)
	}
	if err := EmitVersion(&second, 14); err != nil || !bytes.Equal(first.Bytes(), second.Bytes()) {
		t.Fatal("nondeterministic v14")
	}
	for _, want := range []string{RevisionV14ID, "pc 1\n" + RevisionV13ID,
		"0000000000000000000000000000e002", "00000000000000000000000000005000", "00000000000000000000000000005001",
		"0000000000000000000000000000e004", "0000000000000000000000000000d000", "0000000000000000000000000000d001",
		"0000000000000000000000000000e036", "0000000000000000000000000000e360", "0000000000000000000000000000e366", "00000000000000000000000000005010"} {
		if !strings.Contains(first.String(), want) {
			t.Fatalf("v14 missing %s", want)
		}
	}
	for _, bad := range []string{"go_host", "pulp", "wasm", "workbench", "workshop", "ironclad"} {
		if strings.Contains(strings.ToLower(first.String()), bad) {
			t.Fatalf("language/product contamination %q", bad)
		}
	}
}

func TestEmitV13BindsExactV12AndTargetPlan(t *testing.T) {
	var first, second bytes.Buffer
	if err := EmitVersion(&first, 13); err != nil {
		t.Fatal(err)
	}
	if err := EmitVersion(&second, 13); err != nil || !bytes.Equal(first.Bytes(), second.Bytes()) {
		t.Fatal("nondeterministic v13")
	}
	for _, want := range []string{RevisionV13ID, "pc 1\n" + RevisionV12ID,
		"0000000000000000000000000000e02d", "0000000000000000000000000000c000", "0000000000000000000000000000c001",
		"0000000000000000000000000000e02c", "0000000000000000000000000000e2c0", "0000000000000000000000000000e2c1", "0000000000000000000000000000e2c2", "0000000000000000000000000000c014"} {
		if !strings.Contains(first.String(), want) {
			t.Fatalf("v13 missing %s", want)
		}
	}
	for _, bad := range []string{"go_host", "pulp", "wasm", "workbench", "workshop", "ironclad"} {
		if strings.Contains(strings.ToLower(first.String()), bad) {
			t.Fatalf("target/product contamination %q", bad)
		}
	}
}

func TestEmitV12BindsExactV11AndControlledEffects(t *testing.T) {
	var first, second bytes.Buffer
	if err := EmitVersion(&first, 12); err != nil {
		t.Fatal(err)
	}
	if err := EmitVersion(&second, 12); err != nil || !bytes.Equal(first.Bytes(), second.Bytes()) {
		t.Fatal("nondeterministic v12")
	}
	for _, want := range []string{RevisionV12ID, "pc 1\n" + RevisionV11ID,
		"0000000000000000000000000000e02a", "00000000000000000000000000013000", "00000000000000000000000000013001",
		"0000000000000000000000000000e029", "0000000000000000000000000000e290", "0000000000000000000000000000e291", "0000000000000000000000000000e292", "00000000000000000000000000013100"} {
		if !strings.Contains(first.String(), want) {
			t.Fatalf("v12 missing %s", want)
		}
	}
}

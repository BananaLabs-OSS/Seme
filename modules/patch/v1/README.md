...
unknown-target, unknown-field, failed-precondition, duplicate-write, and
invalid-candidate variants all pass Foundation semantic validation before Patch
...
`diagnostic.seme` converts the applicator's private failure evidence into one
canonical structured `DiagnosticReport`. The adapter optionally publishes that
report atomically on rejection while leaving the requested workspace output
byte-for-byte unchanged. `reference/go/cmd/patch-diagnostic-check` independently
decodes and verifies every required rule, location, and expected/actual pair.

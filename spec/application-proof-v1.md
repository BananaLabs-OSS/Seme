...
The decision now lives in an ordinary Go `WithinLimit` helper. Core Execution
v6 retains `Admit` and `WithinLimit` as separate canonical Functions connected
by `FunctionCall`; lowering emits two Wasm functions and a real call instruction.

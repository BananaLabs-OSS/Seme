...
`projectroundtrip.PublishProjected` is the composition boundary for a genuine
round trip: its caller supplies exactly the tracked paths regenerated by a
language projector (which may choose new presentation paths), while every
nontracked path is recovered at its original path from the detached bundle. It
rejects an empty projection, preserved-path collisions, bundle tampering,
reclassification, preservation drift, unsafe parents, and existing
destinations before publishing the verified staged tree. This materializer

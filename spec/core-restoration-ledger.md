...
| v27 | in progress | Interfaces and bounded dynamic dispatch | `InterfaceType`, `MethodRequirement`, `SatisfactionWitness`, `InterfaceValue`, and `DynamicMethodCall` preserve contract requirements and concrete satisfaction evidence without adopting a source object model. Native Go interfaces and JavaScript structural classes lift and project to byte-identical canonical meaning. Two implementations execute through deterministic witness tags in standalone Wasm and pinned Pulp; unknown tags, malformed Boolean input, receiver-incompatible witnesses, non-requirement calls, and signature mismatches reject. Completion still requires broader method-set forms, revision-bound interface edits, and the cumulative product and external-corpus gates. |
| v28 | in progress | Immutable lexical closures | `FunctionType`, `CaptureBinding`, `CaptureRead`, `ClosureConstruct`, and `IndirectCall` model typed callable values and immutable environments explicitly. Native Go and JavaScript escaping closures lift and project to byte-identical canonical meaning. Positive, negative, independent, and overflowing captures execute through deterministic Wasm and pinned Pulp; a portable closure ABI connects separately lowered producer and consumer artifacts, while unknown tags and invalid type, capture, and arity graphs reject. Completion still requires general closure bodies and capture sets plus the cumulative product and external-corpus gates. |
| v29 | in progress | Mutable closure environments | `MutableCaptureBinding`, `MutableCaptureRead`, `CaptureUpdate`, `Sequence`, `MutableClosureConstruct`, and `StatefulIndirectCall` expose mutation and ordering without hidden canonical state. Native Go and JavaScript mutable closures lift to byte-identical graphs whose first call returns an explicit v26 transition; its returned closure is committed before the second call. Positive, negative, repeated, independent, and overflowing environments agree through deterministic Wasm and pinned Pulp, while malformed capture, update, commit, and call links reject. Completion still requires general mutable capture sets and bodies plus the cumulative product and external-corpus gates. |
| v30 | in progress | Runtime-keyed maps | `MapType`, `EmptyMap`, `MapLookup`, and `MapUpdate` define structural immutable maps with exact signed-i64 keys and values and zero-on-missing lookup. Native Go maps and JavaScript `Map` values lift to a byte-identical generic `Fold` carrying a map accumulator. Repeated, negative, missing, reordered, empty, and 512-element inputs agree through deterministic Wasm and pinned Pulp; 513 elements, malformed ABI layouts, invalid map types, and corrupted lookup/update edges reject. Completion still requires broader map types and operations plus the cumulative product and external-corpus gates. |
